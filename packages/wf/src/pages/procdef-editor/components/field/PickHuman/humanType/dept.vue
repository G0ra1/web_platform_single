...
export default {
  ...
  setup(props, context){
    const selected = computed(() => {
      return DeptCache.value.bizId.split(',').filter(d => d)
    })
    const callback = (f, id, name) => {
      // alert(`${f}${id}${name}`)
      const ids = DeptCache.value.bizId.split(',').filter(d => d)
      const names = DeptCache.value.bizText.split(',').filter(d => d)
      ...
      DeptCache.value.bizId = ids.join(',')
      DeptCache.value.bizText = names.join(',')
      
      DeptCache.value.expreName = `部门(${ids.length})`
    }
    return {
      selected,
      callback
