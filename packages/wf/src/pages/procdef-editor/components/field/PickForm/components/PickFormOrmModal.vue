<template>
  ...
  >
    <template #icon>
      ...
      <nw-icon name="icon-orm" :size="20" color="rgb(103, 194, 58)" />
    </template>
    <template #header>
      <div>
        选择表单字段
...
export default {
  ...
  setup(props, context){
    ...
    // 流程节点表单数据
    const forms = getExtensionRootEl(getRootElement(), 'netwisd:Forms')
    if (!forms.values) {
      forms.values = []
    }
    // 是否加载中
    ...
    const gridFormOption = ref({
      ...
      data: forms.values.filter(d => d.values && d.values.length),
      expandConfig: {
        // labelField
        expandAll: true,
        reserve: true
                      ...
                      slots: {
                        default: ({ row }) => {
                          return [
                            ...
                            type="info"
                            onClick={() => {
                              context.emit('callback', `${fRow.formName}.${row.javaName}`, `${fRow.formNameCh} → ${row.nameCh}`)
                              visible.value = false
                            }}
                            >
                            {{
