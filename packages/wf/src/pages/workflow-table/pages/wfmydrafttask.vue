...
import {wfmydrafttaskList} from "../api/index"
...
export default defineComponent({
  ...
  setup(props,context) {
    ...
    const getData = ()=>{
      spinShow.value = true
      wfmydrafttaskList({...searchFormData.value,page: {current: page.value, size: pageSize.value}}).then(res=>{
    ...
    return {
      ...
      NEmptyShow,
      handle (id) {
        // redirectPage(`/wf/run-skin.html?Action=my_draft&id=${id}`, true)                
      }
    };
  }
});
