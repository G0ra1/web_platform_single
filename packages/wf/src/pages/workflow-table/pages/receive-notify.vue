          ...
          field: "code", title: "单据编号", showHeaderOverflow: true, showOverflow: true, slots: {
            default: ({ row }) => {
              return <span onClick={() => {
                ...
                WfRunModalRef.value.show(`/web-wf/pages/run-skin.html?Action=receive_notify&id=${row.id}`)

                // window.open(
                //   `/web-wf/pages/run-skin.html?Action=receive_notify&id=${row.id}`,
                //   '_blank',
