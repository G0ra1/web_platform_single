...
import {wfSendNotifyTaskList} from "../api/index"
...
export default defineComponent({
  ...
  setup(props,context) {
    ...
    const getData = ()=>{
      spinShow.value = true
      wfSendNotifyTaskList({...searchFormData.value,page: {current: page.value, size: pageSize.value}}).then(res=>{
    ...
    return {
      ...
      NEmptyShow,
      handle (id) {
        // redirectPage(`/wf/run-skin.html?Action=send_notify&id=${id}`, true)                
      }
    };
  }
});
