      ...
      reset
...

        {
          field: "code", title: "单据编号", showHeaderOverflow: true, showOverflow: true, slots: {
            default: ({ row }) => {
              return <span onClick={() => {
                // window.open(`/web-wf/pages/run-skin.html?Action=draft&id=${row.id}`)

                WfRunModalRef.value.show(`/web-wf/pages/run-skin.html?Action=draft&id=${row.id}`)

                // window.open(
                //   `/web-wf/pages/run-skin.html?Action=draft&id=${row.id}`,
                //   '_blank',
