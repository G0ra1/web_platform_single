<template>
  ...
  >
    ...
    <div style="height: 410px;">
      <NwEmployee :value="CircularizeUsers" @callback="callback" />
    </div>
...
export default defineComponent({
  ...
  setup(props: any, context: any){
    ...
    const gridOption = ref<any>({
      ...
      data: CircularizeUsers, // nodeAction
      columns: [
        {
          title: '姓名',
          field: 'userNameCh',
          ...
          slots: {
            default: ({ row }: any) => {
              ...
              <NButton
                ...
                color="#1890ff"
                onClick={() => {
                  const index = CircularizeUsers.value.findIndex(d => d.userId === row.userId)
                  if (index >= 0) {
                    CircularizeUsers.value.splice(index, 1)
                    CircularizeUsers.value = [
                      ...CircularizeUsers.value
                    ]
                  }
                }}
      ...
      return {
        ...
        callback (flag: boolean, userId: string, userNameCh: string, userName: string, row: any) {
          ...
          if (flag) {
            ...
            const index = CircularizeUsers.value.findIndex(d => d.userId === userId)
            if (index >= 0) {
              CircularizeUsers.value.splice(index, 1)
              CircularizeUsers.value = [
                ...CircularizeUsers.value
