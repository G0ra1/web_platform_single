...
export default defineComponent({
  ...
  setup(props, context) {
      ...
      const {
        ...
        { field: 'pageCode', title: '编码', showHeaderOverflow: true, width: '210px' },
        {
          title: '类型', showHeaderOverflow: true, width: '60px',
          slots: {
            default: ({ row }) => {
              ...
              return [
                <NButton
                  ...
                  onClick={() => {
                    ...
                    context.emit('update:pageUrl', row.pageUrl)
                    context.emit('update:pageCode', row.pageCode)
                    context.emit('update:pageName', row.pageName)
                    context.emit('update:appCode', row.appCode)
                    visible.value = false
                  }}
                >{{
                  default: () => '选择'
