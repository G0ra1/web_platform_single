...
const delNews = (row: any) => {
    const delId = row.map((d: any) => d.id).join(',')
    dialogLocal.warning({
        title: '警告',
        content: `是否确定删除所选消息？`,
        positiveText: '确定',
...
export {
    bind, gridRef, filterData, query, refresh, reset, paramsArray, initDialog, initMessage, gridSearchRef, gridEvents, refDetails, delNews
}