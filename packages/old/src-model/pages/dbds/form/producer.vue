<template>
  <n-form :model="model" ref="formRef" :rules="rules">
    <n-grid x-gap="8" :cols="2">
      ...
      <n-grid-item>
        <n-form-item label="生产组名" path="producerGroup">
          <n-input
            placeholder="请输入生产组名"
            v-model:value="model.producerGroup"
          />
        </n-form-item>
      </n-grid-item>
      <n-grid-item v-if="model.mq == 'rabbit'">
...
import { add, edit, producerDetails } from "../api/producer.js";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const model = reactive(
      props.model.type == "rocketmqsc" || props.model.mq == "rocket"
        ? {
            mq: "rocket",
            isEnable: 1,
            port: "9876",
    ...
    const msgSourceChange = (v) => {
      if (v == "msgtmp") {
        model.dbDs = "";
        model.mqSourceContent = "";
        model.method = "";
        model.restfulAddr = "";
        model.restfulDesc = "";
        model.restfulHeder = "";
