<template>
  <n-layout class="nw-page" ref="nwPage">
    <n-layout-header>
      <h1>图片新闻管理</h1>
      ...
      >
        ...
        <template v-slot:search-form="{ searchFormData }">
          <n-form :model="searchFormData">
            ...
            <n-form-item path="title" label="新闻标题">
              <n-input v-model:value="searchFormData.title" @keydown.enter.prevent />
            </n-form-item>
  ...
  <n-drawer
    ...
    :height="200"
    placement="right"
    :to="$refs.nwPage && $refs.nwPage.$el"
  >
    <n-drawer-content
      :title="'图片新闻' + contentTile"
      :header-style="{
        padding: '10px',
        'font-size': '16px',
        'font-weight': 'bold',
    ...
    >
      ...
      <template #footer>
        <n-button size="small" style="margin-right: 5px" @click="formVisible = false">取消</n-button>
