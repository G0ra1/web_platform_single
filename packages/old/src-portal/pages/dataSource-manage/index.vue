...
import { portalDsList, add, edit, del, details } from "./api";
  ...
  setup() {
    ...
    return {
      ...
      columns: reactive([
        ...
        { type: "seq", width: 40 },
        { field: "dsCode", title: "数据源CODE" },
        { field: "dsName", title: "数据源名称", showHeaderOverflow: true },
        {
          field: "dsParamsType",
          title: "数据源类型",
          showOverflow: true,
          slots: {
            default: ({ row }) => {
              return [
                <span>{row.dsParamsType == 0 ? "restFull" : "socket"}</span>,
              ];
            },
          }
        },
        { field: "dsUrl", title: "URL", showOverflow: true },
        { field: "dsReq", title: "请求方式", showOverflow: true,slots: {
            default: ({ row }) => {
              return [
                <span>{row.dsReq == 0 ? "get" : "post"}</span>,
              ];
            },
          } },
        { field: "dsHeaders", title: "headers", showOverflow: true },
