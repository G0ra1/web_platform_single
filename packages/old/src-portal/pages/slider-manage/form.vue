  ...
  <n-spin :show="spinShow">
    <n-form>
      ...
      <n-form-item label="图片标题" path="title">
        <n-input
          placeholder="请输入图片标题"
          v-model:value="picturesSons.title"
        />
      </n-form-item>
      <n-form-item label="图片摘要" path="description">
        <n-input
          placeholder="请输入图片摘要"
          v-model:value="picturesSons.description"
        />
      </n-form-item>
      <n-form-item label="是否外联" path="fileUrl">
        <n-radio
          :checked="picturesSons.isOutLink === 1"
        ...
        <n-radio
          :checked="picturesSons.isOutLink === 0"
          @change="(e) => isOutLinkChange(e, row, _rowIndex)"
          value="0"
          name="isOutLink"
        >
      ...
      <n-form-item
        label="图片上传"
        path="description"
        v-show="picturesSons.isOutLink == 0"
      >
        <n-image width="100" :src="picturesSons.fileUrl" v-if="picturesSons.fileUrl" />
        <n-upload
          :action="uploadUrl"
      ...
      <n-form-item
        label="内容URL"
        path="fileUrl"
        v-show="picturesSons.isOutLink == 1"
      >
        <n-input
          placeholder="请输入内容URL"
          v-model:value="picturesSons.fileUrl"
        />
      </n-form-item>
      <n-form-item
        label="缩略图"
        ...
        >
          ...
          <vxe-table-column field="#" title="缩略图">
            <template #default="{ row }">
              <n-image width="100" :src="row.fileUrl" v-if="row.fileUrl" />
            </template>
          </vxe-table-column>
          <vxe-table-column field="#" title="操作">
            <template #default="{ _rowIndex }">
              <n-button
                ...
                @click="
                  () => {
                    picturesSons.splice(_rowIndex, 1);
                    $refs.filesTable.loadData(picturesSons);
                  }
                "
                >删除</n-button
              >
      ...
      <n-form-item label="内容编辑" path="c">
        <VueUeditorWrap
          v-model="picturesSons.ueditorContent"
          :config="editorConfig"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const picturesSons = reactive(
      model.picturesSons ? { ...model.picturesSons[0] } : { isOutLink: 0 }
    );
  ...
  methods: {
    portalPartLists(portalId) {
      portalPartLists({ portalId, partTypeCode: "PCarousel" }).then((res) => {
    ...
    getModel() {
      this.model.picturesSons = [this.picturesSons];
      return this.model;
    },
    portalSelect(v) {
    ...
    isOutLinkChange(e, v, i) {
      this.picturesSons.isOutLink = e.target.value * 1;
    },
    // addFileOutLink() {
    //   this.picturesSons.push({ isOutLink: 1 });
    //   this.$refs.filesTable.loadData(this.picturesSons);
    // },
