<template>
  <n-form>
    <n-form-item label="门户名称" path="portalId">
      <n-select v-model:value="model.portalId" placeholder="请选择门户名称" :options="portalOptions"
        :fallback-option="(value) => ({ label: '' + model.portalName, value })" :on-update:value="portalSelect" />
    </n-form-item>
    <n-form-item label="所属栏目" path="partId">
      <n-select v-model:value="model.partId" placeholder="请选择所属栏目" :options="partOptions"
        :fallback-option="(value) => ({ label: '' + model.partName, value })" :on-update:value="partSelect" />
    </n-form-item>
    ...
    <n-form-item label="摘要" path="description">
      <n-input placeholder="请输入摘要" v-model:value="model.description" />
    </n-form-item>
    ...
    <n-form-item label="内容编辑" path="c">
      <VueUeditorWrap v-model="model.ueditorContent" :config="editorConfig" editor-id="editor-demo-01"></VueUeditorWrap>
    </n-form-item>
  </n-form>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const msg = ref("");
    const model = reactive(props.model ? props.model : { isDefault: 0 });
  ...
  methods: {
    portalPartLists(portalId) {
      portalPartLists({ portalId, partTypeCode: "PList" }).then((res) => {
        console.log(res);
