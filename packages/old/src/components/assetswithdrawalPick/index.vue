            ...
            <vxe-table-column field title="操作">
              <template v-slot="{ row }">
                <vxe-button type="text" @click="currentChangeEvent(row)"
                  v-if="!multiple && !isAddedByEmpId(row.withdrawalId)" status="primary">选择</vxe-button>
                ...
                <vxe-button type="text" @click="
                  () => {
                    emp.alreadyEmpList = emp.alreadyEmpList.filter(
                      (d) => d.withdrawalId !== row.withdrawalId
                    );
                  }
                " v-if="isAddedByEmpId(row.withdrawalId)" status="warning">取消</vxe-button>
              </template>
            </vxe-table-column>
          </vxe-table>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const isAddedByEmpId = (id) => {
      return emp.alreadyEmpList.map((d) => d.withdrawalId).includes(id);
    };
