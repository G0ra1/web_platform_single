  ...
  setup(props, context) {
    ...
    const gridOptions = reactive({
      ...
      columns: [
        ...
        { field: "applyUserOrgName", title: "申请单位", minWidth: 120, showHeaderOverflow: true, showOverflow: true, },
        ...
        // {
        //   field: "acceptNumber", title: "申请数量", minWidth: 120, showHeaderOverflow: true, showOverflow: true, editRender: { name: 'input', attrs: { placeholder: '请输入名称' } }
        // },
        {
          field: "",
          title: "操作",
          ...
          slots: {
            default: ({ row }) => {
              let btn1 =
                selected.value.map(d => d.assetsId).indexOf(row.assetsId) < 0 ? (
                  <span
              ...
              let btn3 = selected.value.map(d => d.assetsId).indexOf(row.assetsId) >= 0 ? (
                <span
                  style="color:#e6a23c;cursor: pointer;margin-left:10px"
                  onClick={() => {
                    selected.value.splice(selected.value.map(d => d.assetsId).indexOf(row.assetsId), 1)
                  }}
                >
                  取消
                </span>
    ...
    watch(() => props.code, (d) => {
      nextTick(() => {
        ...
        props.value.split(',').forEach((d, i) => {
          selected.value.map(e => e.assetsId).indexOf(d) < 0 ? selected.value.push({
            id: d,
