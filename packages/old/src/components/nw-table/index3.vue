    ...
    <n-layout class="nw-table" ref="tbody">
      <n-layout-header class="nw-table-header">
        <div class="nw-table-header-left" style="position: relative">
          ...
          <n-form inline label-align="left" label-placement="left" :show-feedback="false">
            <n-input-group>
              <n-select :default-value="searchKey" :options="searchOption()" :on-update:value="searchKeyUpdate"
                style="width: 180px" />
              <n-input size="small" v-if="searchType != 'daterange'" v-model:value="searchValue" clearable
                :style="{ width: '100%', height: '32px' }" />
              <n-date-picker v-if="searchType == 'daterange'" :style="{ width: '100%' }" v-model:value="dateRange"
                type="daterange" clearable />
              <n-button type="info" @click="fastSearch" style="height: 32px; padding: 0 8px" size="small">
