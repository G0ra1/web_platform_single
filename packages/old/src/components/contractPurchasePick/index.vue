<template>
  <n-input-group @click="selectContract()">
    <n-input v-model:value="contractCode" disabled :style="{ width: '100%' }" placeholder="选择采购合同"
      class="selectedInput" />
    <n-button style="padding: 0 8px">
      <NwIcon name="icon-sousuo1" :size="15"></NwIcon>
    </n-button>
  </n-input-group>
  <n-modal v-model:show="showModal" :mask-closable="false" preset="dialog" title="选择采购合同信息" content="你确认"
  ...
  setup(props, context) {
    ...
    const gridOptions = reactive({
      ...
      columns: [
        { type: "seq", width: 60 },
        {
          field: "contractCode", title: "合同名称", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "contractCode", title: "合同编号", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "projectName", title: "项目名称", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "type", title: "合同类型", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "state", title: "合同状态", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "contractDate", title: "签订日期", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "startTime", title: "合同开始日期", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "endTime", title: "合同结束日期", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "contractPrice", title: "含税总额", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "orgName", title: "需求单位名称", showHeaderOverflow: true,
          showOverflow: true, width: 120,
        },
        {
          field: "",
    ...
    const onPositiveClick = () => {
      contractCode.value = selected.value.contractCode
      context.emit("update:value", contractCode.value);
      context.emit("update:contractId", selected.value.id);
      context.emit("update:supplierName", selected.value.supplierName);
      context.emit("update:contractTime", selected.value.contractDate + " 00:00:00");
