<template>
  <n-input-group @click="selectContract()">
    <n-input v-model:value="applyCode" disabled :style="{ width: '100%' }" placeholder="选择购置申请" class="selectedInput" />
    <n-button style="padding: 0 8px">
      <NwIcon name="icon-sousuo1" :size="15"></NwIcon>
    </n-button>
  </n-input-group>
  <n-modal v-model:show="showModal" :mask-closable="false" preset="dialog" title="选择购置申请信息" content="你确认"
  ...
  setup(props, context) {
    ...
    const gridOptions = reactive({
      ...
      columns: [
        ...
        {
          field: "code", title: "申请单号", minWidth: 120, showHeaderOverflow: true, showOverflow: true,
        },
