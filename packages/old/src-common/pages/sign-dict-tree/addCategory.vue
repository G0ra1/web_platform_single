          ...
          <n-grid-item>
            <n-form-item label="父级节点" path="parentId">
              <n-tree-select
                :options="parentList"
                size="small"
                :default-value="model.data.parentId"
                placeholder="请选择"
                v-model:value="model.data.parentId"
                filterable
                clearable
              />
            </n-form-item>
          ...
          <n-grid-item>
            <n-form-item label="字典名称" path="dictName">
              <n-input
                size="small"
                placeholder="请输入"
                v-model:value="model.data.dictName"
              />
            </n-form-item>
          </n-grid-item>
          <n-grid-item>
            <n-form-item label="字典code" path="dictCode">
              <n-input
                size="small"
                placeholder="请输入"
                v-model:value="model.data.dictCode"
              />
            </n-form-item>
          </n-grid-item>
        </n-grid>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const parentList = reactive([
      { label: "root", key: "0", id: "0", children: [] },
    ]);
    ...
    const sonFn = (data = {}) => {
      ...
      model.data = { ...data };
      parentList[0].label = props.dictName;
      parentList[0].id = props.dictId;
      parentList[0].key = props.dictId;
      allClassifysFn();
      // for(var key  in data){
      //   model.key = data.key
      // }
    ...
    const allClassifysFn = () => {
      allClassifys(props.dictId).then((res) => {
        const getStr = (list) => {
          list.forEach((row) => {
