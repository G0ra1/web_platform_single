<template>
      ...
      <n-layout-sider collapse-mode="width" :collapsed-width="400" default-collapsed :width="400" bordered>
        <n-space vertical :size="12">
          ...
          <n-spin :show="isLoading">
            <n-tree :data="tree.treeList" :render-prefix="renderPrefix" block-line :pattern="pattern" virtual-scroll
              :default-expanded-keys="['0']" class="treeH" @update:selected-keys="(e) => {
                if (e[0]) {
  ...
  <EditAction ref="editAction" :treeList="tree.treeList" @update:callback="
    () => {
      $refs.nwTable.commitQuery();
  ...
  setup() {
    ...
    return {
      ...
      columns: reactive([
        ...
        {
          field: 'dutyName', title: '职务名称', minWidth: 120, treeNode: true,
          slots: {
            default: ({ row }) => {
              return [
                <span>
        ...
        { field: 'dutyCode', title: '职务编号', minWidth: 120, showHeaderOverflow: true, },
        { field: 'dutyUpName', title: '上级职务', minWidth: 120, showOverflow: true, },
        { field: 'parentDeptName', title: '部门', minWidth: 120, showOverflow: true, },
        {
          field: "status", title: "启用状态", minWidth: 120, showOverflow: true,
          slots: {
            default: ({ row }) => {
              return [
                <span>
        ...
        { field: 'masterNumber', title: '职务人数', minWidth: 120, showOverflow: true, },
        { field: 'partNumber', title: '兼岗人数', minWidth: 120, showOverflow: true, },
        {
          field: '',
          width: 200,
          title: '操作',
