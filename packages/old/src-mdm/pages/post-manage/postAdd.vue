  ...
  <n-spin :show="isLoading">
    <n-form :model="model" ref="formRef" :rules="rules" style="height:600px">
        <n-grid x-gap="8" :cols="2" >
            ...
            <n-grid-item>
                <n-form-item path="postSequId" label="岗位序列">
                     <n-select
                       filterable
            ...
            <n-grid-item>
                <n-form-item path="postTagId" label="岗位标识">
                    <n-select
                        v-model:value="model.postTagId"
                        placeholder="请选择岗位标识"
  ...
  methods:{
    postSequIdChange(postSequId){
        this.dictionaries.mdmPostSequDictList.map((d)=>{
          if(d.id == postSequId) {
              this.model.postSequName =d.postSequName
          }
        })
        //岗位职等
    ...
    postUpParentChange(postUpParentId){
       ...
       postTagIdChange(postTagId){
       
        this.dictionaries.mdmPostTagDictList.map((d)=>{
          if(d.id == postTagId) {
              this.model.postTagName =d.label
          }
        })
      },
