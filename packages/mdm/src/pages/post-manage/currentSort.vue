                ...
                <vxe-toolbar>
                    <template #buttons>
                         <n-grid > 
                             ...
                             <n-grid-item span="8" >
                                <h4 style="color:#2080f0">源目标:  {{model.postName}}</h4>
                             </n-grid-item>
                             <n-grid-item span="8" >
                        ...
                        <vxe-table-column field="" title="操作">
                            <template v-slot="{ row }">
                                ...
                                <vxe-button type="text" 
                                  ...
                                  @click="()=>{
                                    target.targetName = row.postName
...
import {  postSort, postLists } from './api'
...
export default defineComponent({
  ...
  setup (props,context) {
    ...
    const post = reactive({empList:[],empData:[]})
    const pattern = ref('')
    const sonFn = (e) => {
      showModalRef.value = true
      model.id = e.id
      model.postName = e.postName
      isLoading.value = true
      const fn = postLists({
          parentOrgId:e.parentDeptId
      })
      fn.then((res)=>{
            post.postList = res
            post.postData = res
    ...
    const searchEvent1 =() =>{
        ...
        if (filterName) {
            ...
            const searchProps = ['postName', 'parentDeptName',]
            const rest = post.postData.filter(item => searchProps.some(key => XEUtils.toValueString(item[key]).toLowerCase().indexOf(filterName) > -1))
            post.postList = rest.map(row => {
