        ...
        <vxe-table-column field="" title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text" @click="() => {
              alreadyObj = row
            }" v-if="(alreadyObj.formName !== row.formName)" status="primary">选择</vxe-button>
            <vxe-button type="text" @click="() => {
              alreadyObj = {}
            }" v-if="(alreadyObj.formName && alreadyObj.formName == row.formName)" status="warning">取消
            </vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
    </n-spin>
    <vxe-pager :loading="loading" :current-page="page.current" :page-size="page.size" :total="page.totalResult"
