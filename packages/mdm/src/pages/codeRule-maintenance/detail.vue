          ...
          <n-grid x-gap="12" >
            <n-grid-item :span="4">
                <n-form-item label="编号规则名称" path="ruleName">
                    <n-input size="small" placeholder="请输入" v-model:value="formV.ruleName"  disabled/>
                </n-form-item>            
            </n-grid-item>
            <n-grid-item :span="4">
                <n-form-item label="规则类型" path="ruleType">
                    <vxe-select size="mini" v-model="formV.ruleType" style="width:100%" disabled>
                      <vxe-option
                        :key="item.value"
                        :value="item.value"
            ...
            <n-grid-item :span="4" v-if="formV['ruleType'] == 1">
                <n-form-item label="生成时机" path="createOpportunity">
                    <vxe-select size="mini" v-model="formV.createOpportunity" style="width:100%" disabled>
                      <vxe-option
                        :key="item.value"
                        :value="item.value"
          ...
          >
            ...
            >
              <template v-slot:edit="row">
                ...
                >
                  <vxe-option
                    v-for="item in (formV.ruleType == 1 ? encondTypeLists : encondTypeList)"
                    :key="item.value"
                    :value="item.value"
                    :label="item.label"
                  ></vxe-option>
            ...
            <vxe-table-column field="content" title="设置" >
...
<script lang="jsx">
  ...
  import {  creatCodeRule, detail, codeRuleDetail } from './api'
  ...
  export default defineComponent({
    ...
    setup (props,context) {
      ...
      const sonFn = (e) => {
          codeRuleDetail(e.id).then((res)=>{
              rulemaking.value = res.encondRuleDetailList.filter(d=>d.encondType == '自动计数')[0]
              model.value = res.encondRuleDetailList.filter(d=>d.encondType !== '自动计数')
              formV.value = res
          })
      ...
      return {
        ...
        handleCreat(e){
          model.value.push({...rulemaking.value,sort:model.value.length})
