<template>
  ...
  >
    <n-grid :cols="24" :x-gap="24" class="LFormNg">
      ...
      <n-form-item-gi :span="24" label="项目编码" path="productCode">
        <n-input
          placeholder="请输入项目编码"
          v-model:value="dataModel.productCode"
        />
      </n-form-item-gi>
      <n-form-item-gi :span="24" label="项目名称" path="productName">
        <n-input
          placeholder="请输入项目名称"
          v-model:value="dataModel.productName"
        />
      </n-form-item-gi>
...
export default defineComponent({
  ...
  setup() {
    ...
    const detailListGridOption = ref<VxeGridProps<any>>({
      ...
      border: true,
      data: dataModel.value.smmHebooilProductDetailList,
      showFooter: false,
      toolbarConfig: {
        slots: {
          buttons: ({ $grid }: any) => {
            return [
              <NButton
                type="primary"
                onClick={function () {
                  if (!dataModel.value.smmHebooilProductDetailList) {
                    dataModel.value.smmHebooilProductDetailList = [];
                  }
                  dataModel.value.smmHebooilProductDetailList.push({
                    //id: `${new Date().getTime()}`,
                    //clientCode: "0",
                    //clientName: "0",
                  });
                  $grid.reloadData(dataModel.value.smmHebooilProductDetailList);
                }}
              >
                {{
                  default: () => "新增",
            ...
            default: ({ $table, rowIndex }: any) => {
              return [
                <NButton
                  ...
                  type="error"
                  onClick={function () {
                    dataModel.value.smmHebooilProductDetailList!.splice(
                      rowIndex,
                      1
                    );
                    $table.reloadData(
