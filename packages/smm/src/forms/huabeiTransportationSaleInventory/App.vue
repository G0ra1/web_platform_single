      ...
      <n-form-item-gi :span="24" label="" class="LFormTable">
        <vxe-grid ref="detailVxeGrid" v-bind="detailListGridOption" style="
          ...
          <template #zhan_default="{ row, column }">
            <span>{{row.zhan_name}}</span>
          </template>
          <template #zhan_edit="{ row, column }">
            <nw-dic
              :request="{ XHR: dictItemLists, params: 'smm_jzStation' }"
              v-model:value="row.zhan_code"
              v-model:label="row.zhan_name"
              placeholder
              size="small"
              :response="{
                  dataMethod: (r: any) =>{
            ...
            <!-- <nw-dic
              v-else
              :request="{ XHR: dictItemLists, params: 'smm_eljbykc' }"
              v-model:value="row.chuyouku_code"
              v-model:label="row.chuyouku_name"
              placeholder
              size="small"
              :response="{
                  dataMethod: (r: any) =>{
