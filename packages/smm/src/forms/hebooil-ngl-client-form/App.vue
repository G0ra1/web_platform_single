...
export default defineComponent({
  ...
  setup() {
    ...
    const detailListGridOption = ref<VxeGridProps<any>>({
      ...
      border: true,
      data: dataModel.value.smmHebooilNglClientDetailList,
      showFooter: false,
      toolbarConfig: {
        slots: {
          buttons: ({ $grid }: any) => {
            return [
              <NButton
                type="primary"
                onClick={function () {
                  if (!dataModel.value.smmHebooilNglClientDetailList) {
                    dataModel.value.smmHebooilNglClientDetailList = [];
                  }
                  dataModel.value.smmHebooilNglClientDetailList.push({
                    //id: `${new Date().getTime()}`,
                    //clientCode: "0",
                    //clientName: "0",
                  });
            ...
            default: ({ $table, rowIndex }: any) => {
              return [
                <NButton
                  ...
                  type="error"
                  onClick={function () {
                    dataModel.value.smmHebooilNglClientDetailList!.splice(
                      rowIndex,
                      1
                    );
                    $table.reloadData(
