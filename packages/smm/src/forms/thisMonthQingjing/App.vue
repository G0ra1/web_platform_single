        ...
        <n-tab-pane v-for="(item,index) in dataModel.productList" :name="item.code" :tab="item.name" :key="item.code">
          ...
          >
            ...
            <template #kehu_edit="{ row, column }">
              <nw-dic
                :request="{ XHR: qingtingKehuLists, params: item.id }"
                ...
                :response="{
                  dataMethod: (r: any) =>{
                    return r.smmHebooilNglClientDetailList.map((d: any) => ({
                        value: d.clientCode,
                        label: d.clientName,
                    }))
...
import { dictItemLists,smmHebooilNglClients, qingtingKehuLists } from "../../api/index.js";
import ChooseProduct from "../../components/qingtingProduct/index.vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    const kehuListOption = computed<VxeGridProps<any>>(()=>{
      ...
      border: true,
      data: dataModel.value.kehuList.filter((d: any)=>d.projectCode == tabv.value),
      showFooter: true,
      toolbarConfig: {
        slots: {
          buttons: ({ $grid }: any) => {
                ...
                <NwIcon
                  ...
                  onClick={function () {
                    ...
                    dataModel.value.kehuList.push({
                      id: `${new Date().getTime()}`,
                      projectCode: dataModel.value.productList.filter((d:any)=>d.code == tabv.value)[0].code,
                      projectName: dataModel.value.productList.filter((d:any)=>d.code == tabv.value)[0].name,
                    });
                  }}
                />
              </h3>,
    ...
    const kucunOption = computed<VxeGridProps<any>>(()=>{
      ...
      highlightHoverRow: true,
      border: true,
      data: dataModel.value.kucunList.filter((d: any)=>d.projectCode == tabv.value),
      toolbarConfig: {
        slots: {
          buttons: ({ $grid }: any) => {
            return [
      ...
      Db.get("userInfo").then((res: any) => {
        ...
        if (!dataModel.value.fillInTime) {
         ...
         if(!dataModel.value.productList) {
          ...
          smmHebooilNglClients({}).then((r: any)=>{
            r.map((d: any)=>{
              dataModel.value.productList.push({
                id:d.id,
                name:d.projectName,
