...
export default {
  ...
  setup(props, context){
    ...
    const kehuListOption = computed(()=>{
      ...
      border: true,
      data:dataModel.value.smmHebooilNglDetailList? dataModel.value.smmHebooilNglDetailList.filter((d)=>d.productCode == tabv.value):[],
      showFooter: true,
      columns: [
          {
            title: "客户",
    ...
    const kucunOption = computed(()=>{
      ...
      highlightHoverRow: true,
      border: true,
      data: dataModel.value.smmHebooilNglQuantityList? dataModel.value.smmHebooilNglQuantityList.filter((d)=>d.productCode == tabv.value) : [],
      toolbarConfig: {
        slots: {
          buttons: ({ $grid }) => {
            return [
