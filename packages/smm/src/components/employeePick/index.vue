<template>
  ...
  <n-modal v-model:show="showModal" :mask-closable="false" class="employee" preset="dialog" title="选择人员" content="你确认"
    positive-text="确认" negative-text="取消" style="width: 1200px">
    <n-grid>
      <n-grid-item span="6" style="height: 650px" v-if="showSide">
        <div class="sider" style="background:#fff;">
          <n-input v-model:value="pattern" placeholder="搜索" />
          <n-spin :show="leftSpinShow">
            <n-tree placeholder="请选择" :data="tree.treeList" style="height: 650px; padding: 5px 0" block-line
              virtual-scroll :default-expanded-keys="['0']" @update:selected-keys="getEmp" />
          </n-spin>
        </div>
      </n-grid-item>
          ...
          <vxe-toolbar style="height: 50px">
            <template v-slot:buttons>
              <vxe-form>
                ...
                <vxe-form-item>
                  <template #default>
                    <n-input placeholder="搜索" v-model:value="empName">
                    </n-input>
                  </template>
                </vxe-form-item>
                <vxe-form-item>
                  <template #default>
                    <n-button type="info" @click="searchEvent">查询</n-button>
                  </template>
                </vxe-form-item>
              </vxe-form>
            </template>
    ...
    const getEmp = (e) => {
      ...
      if (e) {
        ...
        const fn = empName.value
          ? empLists({ parentOrgId: orgId.value, userNameCh: empName.value })
          : empLists({ parentOrgId: orgId.value });
        fn.then((res) => {
          if (res) {
            res.map((d) => {
