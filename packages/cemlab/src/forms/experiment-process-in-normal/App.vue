...
import { NForm, NInput, NGrid, NFormItemGi, NDatePicker, NScrollbar, NSelect, NSpace, NTimePicker } from "naive-ui";
import { FormModal,cementSlurryBind,slurryBind,prepadBind } from "./App";
import { NwFieldGrid, NwDic, request} from "@platform/main"
const { formRef, dataModel, rules, dataPermits, conditionColumns, resultColumns } = new FormModal();
        ...
        <n-form ref="formRef" :model="dataModel" :rules="rules" size="medium" label-placement="left" label-width="auto"
            ...
            <n-grid :cols="24" :x-gap="24">
                ...
                <n-form-item-gi :span="24">
                    <NwFieldGrid v-model:value="dataModel.cemlabExperimentProcessConditionList"  style="width: 100%"
                        :columns="conditionColumns"
                        :edit-config="{ trigger: 'click', mode: 'row', autoClear: false, showIcon: false }">
                        <template #conditionItemDict="{ row }">
                            <NwDic dictCode="PROCESS_CONDITION" v-model:value="row.conditionItemDict"
                                v-model:label="row.conditionItem"></NwDic>
                        </template>
                        <template #conditionItemDictDefault="{ row }">
                            {{ row.conditionItem }}
                        </template>
                        <template #conditionVal="{ row }">
                            <n-input placeholder="数值" v-model:value="row.conditionVal" />
                        </template>
                    </NwFieldGrid>
                </n-form-item-gi>
                <n-form-item-gi :span="24" label="">
                ...
                <n-form-item-gi :span="24">
                    <vxe-grid v-bind="actualBind" style="width:100%">
                        <template #actualExperimentTemperature_edit="{ column }">
                            <vxe-input v-model="dataModel.actualExperimentTemperature" @change="(v: any) => actualChange(v, column)"></vxe-input>
                        </template>
                        <template #actualExperimentTemperature_defalut>
                            {{ dataModel.actualExperimentTemperature }}
                        </template>
                        <template #actualExperimentPressure_edit="{ column }">
                            <vxe-input v-model="dataModel.actualExperimentPressure" @change="(v: any) => actualChange(v, column)"></vxe-input>
                        </template>
                        <template #actualExperimentPressure_defalut>
                            {{ dataModel.actualExperimentPressure }}
                        </template>
                        <template #actualHeatingTime_edit="{ column }">
                            <vxe-input v-model="dataModel.actualHeatingTime" @change="(v: any) => actualChange(v, column)"></vxe-input>
                        </template>
                        <template #actualHeatingTime_defalut>
                            {{ dataModel.actualHeatingTime }}
                        </template>
                    </vxe-grid>
                </n-form-item-gi>
                <n-form-item-gi :span="24">
                    <NwFieldGrid v-model:value="dataModel.cemlabExperimentProcessResultList" style="width: 100%"
                        :columns="resultColumns"
                        :edit-config="{ trigger: 'click', mode: 'row', autoClear: false, showIcon: false }">
                        <template #resultItemDict="{ row }">
                            <NwDic dictCode="DETECTION_ITEM" v-model:value="row.resultItemDict"
                                v-model:label="row.resultItem"></NwDic>
                        </template>
                        <template #resultItemDictDefault="{ row }">
                            {{ row.resultItem }}
                        </template>
                        <template #resultVal="{ row }">
                            <n-input placeholder="数值" v-model:value="row.resultVal" />
                        </template>
                        <template #resultValDefault="{ row }">
                            {{ row.resultVal }}
                        </template>
                    </NwFieldGrid>
                </n-form-item-gi>
                ...
                <n-form-item-gi :span="24">
                    <vxe-grid v-bind="slurryBind" style="width:100%">
                        <template #slurrySpeed3_edit="{ row, column }">
                            <vxe-input v-model="row.slurrySpeed3"
                                @change="(v: any) => slurryChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #slurrySpeed6_edit="{ row, column }">
                            <vxe-input v-model="row.slurrySpeed6"
                                @change="(v: any) => slurryChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #slurrySpeed100_edit="{ row, column }">
                            <vxe-input v-model="row.slurrySpeed100"
                                @change="(v: any) => slurryChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #slurrySpeed200_edit="{ row, column }">
                            <vxe-input v-model="row.slurrySpeed200"
                                @change="(v: any) => slurryChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #slurrySpeed300_edit="{ row, column }">
                            <vxe-input v-model="row.slurrySpeed300"
                                @change="(v: any) => slurryChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #slurrySpeed600_edit="{ row, column }">
                            <vxe-input v-model="row.slurrySpeed600"
                                @change="(v: any) => slurryChange(v, row, column)" type="number"></vxe-input>
                        </template>
                ...
                <n-form-item-gi :span="24">
                    <vxe-grid v-bind="prepadBind" style="width:100%">
                        <template #prepadSpeed3_edit="{ row, column }">
                            <vxe-input v-model="row.prepadSpeed3"
                                @change="(v: any) => prepadChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #prepadSpeed6_edit="{ row, column }">
                            <vxe-input v-model="row.prepadSpeed6"
                                @change="(v: any) => prepadChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #prepadSpeed100_edit="{ row, column }">
                            <vxe-input v-model="row.prepadSpeed100"
                                @change="(v: any) => prepadChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #prepadSpeed200_edit="{ row, column }">
                            <vxe-input v-model="row.prepadSpeed200"
                                @change="(v: any) => prepadChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #prepadSpeed300_edit="{ row, column }">
                            <vxe-input v-model="row.prepadSpeed300"
                                @change="(v: any) => prepadChange(v, row, column)" type="number"></vxe-input>
                        </template>
                        <template #prepadSpeed600_edit="{ row, column }">
                            <vxe-input v-model="row.prepadSpeed600"
                                @change="(v: any) => prepadChange(v, row, column)" type="number"></vxe-input>
                        </template>
                ...
                <n-form-item-gi :span="12" label="仪器">
                    <n-select
                        ...
                        @update-value="(d,op:any)=>{dataModel.deviceCode=op.deviceCode,dataModel.deviceName = op.deviceName}"
                    ></n-select>
                </n-form-item-gi>
