/**
 * @Description 实验管理-实验任务-查看内部通知任务详情
...
class FormModal extends AbstractForm {
  ...
  experimentTaskDataModel = ref<CemlabExperimentProcess>(
    new CemlabExperimentProcess()
  );
  //验证规则
  rules = ref<FormRules>({});
  ...
  async validate() {
    const res = [];
    res.push(...(await super.validate()));
    for (let i = 0; i < this.sonForms.value.length; i++) {
      await this.sonForms.value[i]
        .validate()
        .then(() => {})
...
const cementTypeBind = reactive<VxeGridProps>({
  ...
  columns: [
    ...
    {
      field: "recipeMaterialsDict",
      title: "配方材料",
      editRender: {},
      slots: {
        edit: "recipeMaterialsDict_edit",
        default: "recipeMaterialsDict_edit",
      },
    },
    {
      field: "recipeQuantity",
...
export { FormModal, bind, cementTypeBind };
