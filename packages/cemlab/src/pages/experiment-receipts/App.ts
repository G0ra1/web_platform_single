...
const { bind, gridRef, filterData, query, reset } = new VxeHelper.VxeGridPaging(
    {
        columns: [
            ...
            { field: 'experimentUserName', title: '实验人员', showHeaderOverflow: true, minWidth: '160px', align: 'center' },
            {
                ...
                slots: {
                    default: ({ row }: any) => {
                        return XEUtils.toDateString(row.finishDate, 'yyyy-MM-dd')
                    }
                }
            },
            {
            ...
            { field: 'createUserName', title: '创建人', showHeaderOverflow: true, minWidth: '160px', align: 'center' },
            {
                ...
                slots: {
                    default: ({ row }: any) => {
                        return XEUtils.toDateString(row.createTime, 'yyyy-MM-dd')
                    }
                }
            },
            {
                ...
                slots: {
                    default: ({ row }: any) => {
                        let edit = h(
                            NwFunctionPredefine,
                            { code: "experimentReceiptsFormEdit", params: [row] }
                        );
                        let outEdit = h(
                            NwFunctionPredefine,
                            { code: "experimentReceiptsOutFormEdit", params: [row] }
                        );
                        let del = h(
                            NwFunctionPredefine,
                            { code: "experimentReceiptsFormDel", params: [row] }
                        );
                        if (row.type == 1) {
                            return [edit, del]
                        }
