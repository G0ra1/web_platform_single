    ...
    >
      <n-layout-header class="nw-table-header">
        <div class="nw-table-header-left">
          ...
          {{ selected.length ? `已选择${selected.length
          }项` : '' }}
        </div>
...
// import {  organList ,rightList} from '../../pages/organization-manage/api'
    ...
    const gridOptions = reactive({
      ...
      treeConfig: {
        ...
        loadMethod:({ row } ) => {
              //异步加载子节点
              // return new Promise(resolve => {
              //   rightList({status:1,parentId:row.id,isDefault:0,}).then((res)=>{
              //     resolve(res)
              //   })
              // })
        }
