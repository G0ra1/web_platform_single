<template>
    ...
    >
    <n-drawer-content title="阅卷" style="background: #efefef;"  :header-style="{
      'padding': '10px',
      'font-size': '16px',
      'font-weight': 'bold',
        ...
        <div class="examPaperBorder" ref="examPaperBorder">
                    ...
                    >
                        ...
                        <template #footer>
                           <div style="background:rgb(247 244 244);padding:5px">
                            <p :style="`${item.isCorrect==1?'color:#18a058':'color:red'}`">
                               {{item.isCorrect==1?'答对了':(item.isCorrect==0 ? '答错了' :'')}}
                            </p>
                             <p>
                               学员得分： {{item.score}}
                            </p>
                        ...
                        <div style="display:flex;">
                            <div style="flex:1">
                                ...
                                <n-radio-group :value="item.userAnswer"  disabled name="radiogroup" @update:value="(value)=>{formData.questionList[index].userAnswer = value}" v-if="item.questionCode == 3">
                            ...
                            <div style="min-width:200px;min-height:110px;line-height:50px;background:rgb(247 244 244);padding:5px;text-align:center;margin-left:15px">
                                 <n-radio-group  :value="item.isCorrect" 
                                    @update:value="(value)=>{
                                        formData.questionList[index].isCorrect = value;
                                        value == 1? formData.questionList[index].score = item.questionScore : formData.questionList[index].score = 0
                                    }">
                                            <n-radio  :value="1" >
                                                对
                                            </n-radio>
            ...
            <div class="operatingSpaceMessage">
                    ...
                    <div style="border-bottom: 1px dashed;">
                        ...
                        <span style="font-size:16px;font-weight:bolder;color:red"> {{sum(formData.questionList ? formData.questionList.map(d=>d.score) : [])}}</span>
                    </div>
                     <div style="border-bottom: 1px dashed;">
                        <span style="display:block;">考试状态</span>
                        <span :style="`font-weight:bolder;color:${sum(formData.questionList ? formData.questionList.map(d=>d.score) : []) < formData.specialExamQualifiedScore ?'rgb(255, 75, 80)':'rgb(3, 221, 109)'}`">
                            {{sum(formData.questionList ? formData.questionList.map(d=>d.score) : []) < formData.specialExamQualifiedScore ? '未通过' :'通过'}}
                        </span>
                    </div>
                    <div>
...
<script>
  ...
  import { markPaperDetail, savePaper } from './api'
  ...
  export default defineComponent({
    ...
    setup (props,context) {
      ...
      const submitAction = () =>{
        spinShow.value = true
        savePaper(formData.value).then(r=>{
            if(r){
