    ...
    <div class="personalMessage">
      ...
      <div class="second">
         <div style="width:100%; background: #FFF;">
          <div style="height:35px;line-height:35px;border-bottom:1px dashed black;width:98%;margin:auto" @click.stop> 
            排序 <n-radio-group v-model:value="newOrHeartV" name="radiobuttongroup" @update:value="()=>{newOrHeartC()}">
                    <n-space>
                      ...
                      <span  v-if="item.value == `sj`" @click.stop="newOrHeartC">{{ item.name }}   
                        <nw-icon
                          :name="(fromData.orderBySort=='asc')?`icon-shengjiangxu-shengxu-copy1` : 'icon-shengjiangxu-shengxu-copy11'"
                          :size="12"
                  ...
                  <div style="float:right">
                    <vxe-input v-model="fromData.specialName"  placeholder="输入名称查询" @input="getInformData"></vxe-input>
                  </div>
           </div>
           <div class="tag" style="width:98%;margin:auto">
             <div @click.stop class="tagTop" :style="`overflow: hidden;width:100%;${isShow?'':'height:30px'}`">
            ...
            <!-- <n-collapse arrow-placement="right">
                ...
                <n-collapse-item title="更多 " name="1">
                    <template #header>
                      <div @click.stop class="tagTop">
                    标签  <n-radio-group v-model:value="tagV" name="radiobuttongroup1" @update:value="classTagC">
                            <n-space :size='1'>
                                <n-radio-button
                                  size="small"
                                  v-for="item in (classTag)"
         ...
         <n-spin :show="spinShow">
          <div style="padding:5px 0" v-if="courseList.length">
           ...
           <n-grid x-gap="12" y-gap="5" :cols="4" >
                <n-gi v-for="item in courseList" :key="item.id" class="content" @click="()=>{showSpecialDetail(item)}">
                  <div class="img" >
                    <img  v-lazy="item.fileUrl" style="width:100%;height:200px;border-radius: 6px;">
                  </div>
                  <div style="height:110px;width:90%;margin:auto">
                    
                      <n-grid :cols="1" style="height:30px;line-height:30px;">
                        <n-gi>
                          <span style="font-size:18px;font-weight:bolder;display:inline-block;white-space: nowrap; overflow: hidden; text-overflow: ellipsis;width:100%" :title="item.specialName"> {{item.specialName}}</span>
                        </n-gi>
                      </n-grid>
                      
                     <div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                        <p style="height:20px;line-height:20px">学时：{{item.studyTimeText}} | {{item.lessCount}} 个课程 </p>
                       
                        <p style="height:20px;line-height:20px"> 
                          <span style="display:inline-block;width:50%;font-weight:bolder">
                             {{item.apply==0 ? '待申请' : (item.apply==2 ? '申请中' :'')}}
                          </span>
...
import { courseClassification, specialList, dictItemLists,specialDetail  } from  '../api'
export default defineComponent({
  ...
  setup(props,context) {
    ...
    const fromData = ref({labelCode:'',orderBySort:'desc'})
    const tagV = ref('')
    ...
    const newOrHeartC = () =>{
      if( newOrHeartV.value == 'zuixin'){
         fromData.value.orderByField = "createTime"
         fromData.value.orderBySort = "desc"
      } else if( newOrHeartV.value == 'zuire'){
         fromData.value.orderByField = 'hits'
         fromData.value.orderBySort = "desc"
      } else {
        fromData.value.orderByField = "createTime"
        fromData.value.orderBySort = (fromData.value.orderBySort == "desc") ? 'asc' : 'desc'
      }
    ...
    const getInformData = () =>{
       spinShow.value = true
        specialList({page: {current: page.value, size: 12,},...fromData.value}).then((res)=>{
   ...
   //计算中文字符长度
    ...
    onMounted(()=>{
      getInformData()
       ...
       dictItemLists('STUDY_PLAN_TYPE').then((r)=>{
