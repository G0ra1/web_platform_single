        ...
        <n-grid-item span="6" style="height: 800px;" v-if="showSide">
            <div class="sider">
                ...
                <n-spin :show="leftSpinShow" style="padding-top: 6px">
                    ...
                    >
                        <vxe-table-column field="dictItemName" title="学习资料"></vxe-table-column>
...
import { dictItemLists,learningQuerys  } from './api'
  ...
  setup (props,context) {
    ...
    const sonFn = (e=[]) => {
      ...
      leftSpinShow.value = true
        dictItemLists('STUDY_MATERIAL_TYPE').then((res)=>{
    ...
    const  getTopic = (id)=>{
        spinShow.value = true
        learningQuerys({labelCode:id,}).then((res)=>{
            course.topicList = res.map((d)=>{
                d.materialsId = d.id
                d.materialsName = d.marterialsName
                d.materialsTypeCode = d.labelCode
                d.materialsTypeName = d.labelName
