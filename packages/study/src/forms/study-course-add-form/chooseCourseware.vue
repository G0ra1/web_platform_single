            ...
            <div class="sider">
                ...
                <n-spin :show="leftSpinShow" style="padding-top: 6px">
                    <vxe-table
                        ...
                        :data="course.types"
                    >
                        <vxe-table-column field="dictName" title="课件分类" tree-node></vxe-table-column>
            ...
            <n-grid-item :span="showSide ? 18 : 24" style="height: 800px;">
                 <n-spin :show="spinShow" >
                    ...
                    >
                        ...
                        <vxe-column field="labelName" title="课件分类" ></vxe-column>
                        <vxe-column field="couType" title="课件类型"  >
                          <template v-slot="{ row }">
                             {{[
                                 {label:'音频',value:1},
                                {label:'视频',value:2},
...
import { querys,allClassifys  } from './api'
  ...
  setup (props,context) {
    ...
    const sonFn = (e=[]) => {
      ...
      leftSpinShow.value = true
        // dictItemLists('STUDY_COURSEWARE_TYPE').then((res)=>{
        //     course.studyExamDatabaseDefList = res
        //     leftSpinShow.value = false
        // })
      allClassifys('study_cou_type').then((r)=>{
        leftSpinShow.value = false
        course.types = r
       })
    ...
    const  getTopic = (id)=>{
        spinShow.value = true
        querys({labelCode:typeCode.value,}).then((res)=>{
