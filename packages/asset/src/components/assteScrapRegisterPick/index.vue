      ...
      columns: [
        ...
        { field: "usableNumber", title: "可用数量", minWidth: 120, showHeaderOverflow: true, showOverflow: true, },
        {
          ...
          slots: {
            default: ({ row }) => {
              let btn1 =
                selected.value.map(d => d.applyDetailId).indexOf(row.applyDetailId) < 0 ? (
                  <span
              ...
              let btn3 = selected.value.map(d => d.applyDetailId).indexOf(row.applyDetailId) >= 0 ? (
                <span
                  style="color:#e6a23c;cursor: pointer;margin-left:10px"
                  onClick={() => {
                    selected.value.splice(selected.value.map(d => d.applyDetailId).indexOf(row.applyDetailId), 1)
                  }}
                >
                  取消
                </span>
          ...
          query: ({ page }, currentPage) => {
            return new Promise((resolve) => {
              getDataList({
                ...
                resolve({
                  total: res.total,
                  result: res.map(d => { d.applyDetailId = d.id; d.id = null; return d })
                });
              });
            });
          },
