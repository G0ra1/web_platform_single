      ...
      <n-grid-item :span="showSide ? 18 : 24" style="height: 520px">
        <n-spin :show="spinShow">
          ...
          <vxe-table resizable highlight-current-row ref="xTable" :row-config="{ keyField: 'rowId' }" :height="'480px'"
            ...
            <vxe-column field="notRefundNumber" width="135" title="未退库数量"></vxe-column>
            ...
            <vxe-table-column field title="操作">
              <template v-slot="{ row }">
                <vxe-button type="text" @click="currentChangeEvent(row)"
                  v-if="!multiple && !isAddedByEmpId(row.acceptResultId)" status="primary">选择</vxe-button>
                ...
                <vxe-button type="text" @click="
                  () => {
                    emp.alreadyEmpList = emp.alreadyEmpList.filter(
                      (d) => d.acceptResultId !== row.acceptResultId
                    );
                  }
                " v-if="isAddedByEmpId(row.acceptResultId)" status="warning">取消</vxe-button>
              </template>
            </vxe-table-column>
          </vxe-table>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const isAddedByEmpId = (id) => {
      return emp.alreadyEmpList.map((d) => d.acceptResultId).includes(id);
    };
    ...
    const getEmp = () => {
      spinShow.value = true;
      empLists({ applyUserId: props.applyUserId }).then((res) => {
        if (res) {
          res.records.map((d) => {
            d.acceptResultId = d.id;
            d.id = null;
          });
        }
