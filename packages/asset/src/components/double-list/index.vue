...
import { rowKey, onSelectChange, onPageChange, rehandleChange, sourceColumns, targetColumns, pages, params } from './index'
import { sourceData, targetData, isLoading, selectedRowKeys, targetClickEvent, enterEvent, search } from './index'
...
const submitEvent = () => {
    emit('callback', cloneDeep(targetData.value));
...
<template>
    <n-modal :show="porps.show" @update:show="(d) => update(d)" preset="dialog" :mask-closable="true" :closable="true"
        style="width: 1000px; height: 800px; padding: 0" title="请选择物资信息" @after-enter="() => enterEvent(porps)">
        <n-space vertical>
