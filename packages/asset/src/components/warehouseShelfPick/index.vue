<template>
  <n-input-group @click="selectContract()">
    <n-input v-model:value="shelfName" disabled :style="{ width: '100%' }" placeholder="选择货架" class="selectedInput" />
    <n-button style="padding: 0 8px">
      <NwIcon name="icon-sousuo1" :size="15"></NwIcon>
    </n-button>
  </n-input-group>
  <n-modal v-model:show="showModal" :mask-closable="false" preset="dialog" title="选择货架信息" positive-text="确认"
  ...
  setup(props, context) {
    ...
    const gridOptions = reactive({
      ...
      columns: [
        ...
        {
          field: "warehouseName", title: "所属仓库", showHeaderOverflow: true,
          showOverflow: true
        },
        {
          field: "",
          ...
          query: ({ page }, currentPage) => {
            return new Promise((resolve) => {
              contractPurchaseList({
                ...
                resolve({
                  total: res.total,
                  result: res.records.map(d => { d.warehouseName = props.warehouseName; return d })
                });
              });
            });
          },
    ...
    const shelfName = ref('')
    if (props.value) {
      shelfName.value = props.value
    }
    const onPositiveClick = () => {
      context.emit("update:value", selected.value.shelfName);
