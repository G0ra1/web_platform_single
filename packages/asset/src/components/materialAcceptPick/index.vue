<template>
  <n-input-group @click="selectContract()">
    <n-input v-model:value="applyCode" disabled :style="{ width: '100%' }" placeholder="选择物资领用" class="selectedInput" />
    <n-button style="padding: 0 8px">
      <NwIcon name="icon-sousuo1" :size="15"></NwIcon>
    </n-button>
  </n-input-group>
  <n-modal v-model:show="showModal" :mask-closable="false" preset="dialog" title="选择物资领用信息" content="你确认"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const onPositiveClick = () => {
      ...
      context.emit("onChange", selected.value.map(d => d.id).toString());
