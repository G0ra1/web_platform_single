          ...
          slots: {
            default: ({ row }) => {
              ...
              let btn2 = props.multiple && !isAddedByEmpId(row.itemId) ? <span
                style="color:#409eff;cursor: pointer;margin-left:10px"
                onClick={() => {
                  console.log(emp.alreadyEmpList.map(d => d.itemId == row.itemId).length + '' + +new Date())
                  emp.alreadyEmpList.push({ ...row, rowId: emp.alreadyEmpList.map(d => d.itemId == row.itemId).length + '' + +new Date() });
