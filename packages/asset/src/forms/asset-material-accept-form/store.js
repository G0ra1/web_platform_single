...
window.DM_SET_VALUE = async function (v) {
  ...
  if (v.materialAcceptAssetsList && v.materialAcceptAssetsList.length > 0) {
    v.materialAcceptAssetsList = enlargeData(v.materialAcceptAssetsList)
    detailList.value = v.materialAcceptAssetsList
  }
  dataModel.value = v;
};
// 设置表单字段权限
...
window.DM_GET_VALUE = function () {
  dataModel.value.materialAcceptAssetsList = reduceData(detailList.value)
