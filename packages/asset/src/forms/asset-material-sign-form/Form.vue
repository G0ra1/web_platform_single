  ...
  <div class="my-form-box">
    ...
    <n-form :model="dataModel" :rules="rules" ref="formRef" label-placement="left" label-align="left" :label-width="90"
      ...
      <n-grid :cols="12" :x-gap="24">
        ...
        <n-form-item-gi :span="6" label="签收编号" path="code">
          <n-input placeholder="签收编号" v-model:value="dataModel.code" disabled />
        </n-form-item-gi>
        ...
        <n-form-item-gi :span="12" label="" path="">
          <div style="width: 100%">
            ...
            <vxe-table ref="detailTable" show-overflow border :data="assetsList" :column-config="{ resizable: true }"
              ...
              <vxe-column field="distributeNumber" title="派发数量" width="135">
              </vxe-column>
              <vxe-column field="signNumber" title="签收数量" width="135" :edit-render="{}">
                <template #edit="{ row, column }">
                  <vxe-input v-model="row.signNumber" type="number"></vxe-input>
                </template>
              </vxe-column>
              <vxe-column field="signStatus" title="签收状态" width="135" :edit-render="{}">
                <template #edit="{ row }">
                  <vxe-select v-model="row.signStatus" transfer>
                    <vxe-option :key="1" :value="1" label="签收">
                    </vxe-option>
                    <vxe-option :key="0" :value="0" label="拒签">
                    </vxe-option>
                  </vxe-select>
                </template>
                <template #default="{ row }">
                  <span>{{ row.signStatus === 1 ? "签收" : row.signStatus === 0 ? "拒签" : "" }}</span>
                </template>
              </vxe-column>
...
import materialAcceptPick from "../../components/materialAcceptPickqianshou/index.vue";
import {
  useMessage,
  NForm,
  NGrid,
...
import { dictItemLists, getAssetsList, wuziType } from "./api/index.js";
...
export default defineComponent({
  ...
  setup() {
    ...
    const materialAcceptChange = (sourceId) => {
