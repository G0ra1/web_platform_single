...
const choose = () => {
    dataModel.value.assetParams.inventoryId = dataModel.value.inventoryPlanId;
...
const callAsset = (data: Array<any>) => {
    dataModel.value.detailList = data.map((item) => {
            ...
            item.inventoryNumber = item.paperNumber;
            item.inventoryPlanDetailId = item.id;
            item.inventoryPlanId = dataModel.value.inventoryPlanId;
            delete item.id;
            return new AssetInventoryTaskDetail(item)
        }  
    )
}
                ...
                <NwFieldGrid v-model:value="dataModel.detailList" style="width: 100%" :edit-enabled="true"
                    ...
                    <template #buttons>
                        ...
                        <SelAssetList v-model:show="detailListShow" :columns="assetList"
                                ...
                                :url="['/asset/inventoryPlan/getDetailByIdAndUser','/asset/inventoryTask/getDetailByIds']" :params="dataModel.assetParams"
                                @callback="callAsset" />
                            <!-- <SelAssetListTest 
                                ref="xTree"
                                v-model:show="detailListShow" :columns="assetList"
                                :url="['/asset/assetAssets/list']" :params="dataModel.assetParams"
                                :title ="'选择明细信息'"
                                @callback="callAsset" /> -->
                    </template>
                    ...
                    <template #inventoryNumber="{ row }">
                        ...
                        <n-input-number
                            :min="0"
                            v-model:max="row.paperNumber"
                            v-model:value="row.inventoryNumber"
                        ></n-input-number>
                    </template>
                    <template #inventoryReason="{ row }">
                        <!-- {{ row.inventoryReason}} -->
                        <n-input v-model:value="row.inventoryReason"></n-input>
                    </template>
                </NwFieldGrid>
            </n-scrollbar>
        </n-form-item>
