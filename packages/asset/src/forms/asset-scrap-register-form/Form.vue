<template>
  <div class="my-form-box">
    ...
    <n-form :model="dataModel" :rules="rules" ref="formRef" label-placement="left" label-align="left" :label-width="90"
      ...
      <n-grid :cols="12" :x-gap="24">
        ...
        <n-form-item-gi :span="6" label="计划编号" path="code">
          <n-input placeholder="计划编号" v-model:value="dataModel.code" disabled />
        </n-form-item-gi>
        <n-form-item-gi :span="6" label="登记单位" path="applyUserOrgName">
          <n-input placeholder="登记单位" v-model:value="dataModel.applyUserOrgName" disabled />
        </n-form-item-gi>
        ...
        <n-form-item-gi :span="6" label="编制人" path="applyUserName">
          <n-input-group>
            <n-input v-model:value="dataModel.applyUserName" class="selectedInput" disabled />
          </n-input-group>
        </n-form-item-gi>
        <n-form-item-gi :span="6" label="报废申请" path="applyTime">
          <assetApplyListPick v-model:value="dataModel.applyId" v-model:code="dataModel.applyCode"></assetApplyListPick>
        </n-form-item-gi>
        ...
        <n-form-item-gi :span="12" label="" path="">
          <h5 class="card-title">报废登记明细</h5>
        </n-form-item-gi>
        <n-form-item-gi :span="12" label="" path="">
          <div style="width: 100%">
            <span style="height: 24px; line-height: 24px">报废登记明细</span>
            <n-button style="
                height: 24px;
                width: 24px;
                line-height: 24px;
            ...
            <vxe-table ref="detailTable" show-overflow border :data="assetsList" :column-config="{ resizable: true }"
              ...
              <vxe-table-column field="assetsSumAmount" width="135" title="资产原值"></vxe-table-column>
              <vxe-table-column field="scrapDate" width="135" title="报废时间" :edit-render="{}">
                <template #edit="{ row }">
                  <vxe-input v-model="row.scrapDate" type="date" placeholder="请选择日期" transfer></vxe-input>
                </template>
              </vxe-table-column>
              <vxe-column field="scrapNumber" width="135" title="申请报废数量"></vxe-column>
              <vxe-column field="scrapRegisterNumber" width="135" title="登记数量" :edit-render="{}">
                <template #edit="{ row, column }">
                  <vxe-input v-model="row.scrapRegisterNumber" type="number" @change="dataChangeEvent({ row, column })"
                    :min="0" :max="row.scrapNumber">
                  </vxe-input>
                </template>
              </vxe-column>
              <vxe-column field="cause" width="135" title="报废原因" :edit-render="{}">
                <template #edit="{ row, column }">
  ...
  <assteScrapRegisterPick ref="assteScrapRegisterPickRef" @update:callback="callbackSetItem"
    :applyIds="dataModel.applyId">
  </assteScrapRegisterPick>
...
import assteScrapRegisterPick from "../../components/assteScrapRegisterPick/index.vue";
...
import assetApplyListPick from "../../components/assetApplyListPick/index.vue";
