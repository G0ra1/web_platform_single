...
window.DM_SET_VALUE = async function (v) {
  ...
  if (!v.assetSourceName) {
    v.assetSourceName = "请选择"
  }
  if (v.detailList && v.detailList.length > 0) {
    v.detailList = enlargeData(v.detailList)
    detailList.value = v.detailList.map(d => {
      d.storageTime = d.storageTime ? d.storageTime.slice(0, 10) : null
      return d
    })
  }
  console.log(detailList.value, 'detailListdetailListdetailList')
  dataModel.value = v;
};
// 设置表单字段权限
...
window.DM_GET_VALUE = function () {
  dataModel.value.detailList = reduceData(detailList.value.map(d => {
    if (d.storageTime && d.storageTime.length == 10) {
      d.storageTime = d.storageTime + ' 00:00:00'
    }
    return d
  }))
  console.log(cloneDeep(dataModel.value), 'cloneDeep(dataModel.value)')
