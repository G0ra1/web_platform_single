<template>
  <div class="my-form-box">
    ...
    >
      <n-grid :cols="12" :x-gap="24">
        ...
        <n-form-item-gi :span="6" label="物资来源" path="assetSource">
          <n-input placeholder="物资来源" v-model:value="dataModel.assetSourceName" disabled />
        </n-form-item-gi>
        ...
        <!-- <n-form-item-gi
          :span="6"
          label="验收信息"
          v-if="dataModel.assetSource == 1"
          path="assetSource"
        >
          <yanshouPick
            ref="yanshouPickRef"
            v-model:id="dataModel.acceptanceId"
            v-model:value="dataModel.acceptanceCode"
          ></yanshouPick>
        </n-form-item-gi> -->
        <n-form-item-gi
          :span="12"
        ...
        <n-form-item-gi :span="12" label="" path="">
          <div style="width: 100%">
            ...
            >
            <n-button
              style="height: 24px;width: 24px;line-height: 24px;margin-left: 10px;"
              @click="
                () => {
                  addAsset();
                }
            ...
            >
              ...
              >
                <template v-slot="{ row }">
                  <span>{{
                    wuziTypeList.find((d) => d.dictItemCode == row.itemType)
                      ? wuziTypeList.find((d) => d.dictItemCode == row.itemType)
                          .dictItemName
                      : ""
                  }}</span>
              ...
              <vxe-column field="specs" width="135" title="规格"> </vxe-column>

              <vxe-column field="classifyTypeName" title="资产分类" width="135" :edit-render="{}" transfer>
                <template #edit="{ row }">
                  <NwDic dictCode="asset_classify_type" v-model:value="row.classifyTypeCode"
                    v-model:label="row.classifyTypeName"></NwDic>
                </template>
              </vxe-column>
              
              <vxe-column field="assetsCode" width="135" title="资产编号" :edit-render="{}">
                <template #edit="{ row }">
                  <vxe-input v-model="row.assetsCode" type="text">
                  </vxe-input>
                </template>
              </vxe-column>
              <vxe-column field="assetSelfCode" width="135" title="资产自编号" :edit-render="{}">
                <template #edit="{ row }">
                  <vxe-input v-model="row.assetSelfCode" type="text">
                  </vxe-input>
                </template>
              </vxe-column>
              <vxe-column field="assetFinanceCode" width="135" title="资产财务编号" :edit-render="{}">
                <template #edit="{ row }">
                  <vxe-input v-model="row.assetFinanceCode" type="text">
                  </vxe-input>
                </template>
              </vxe-column>
              ...
              >
                <template #edit="{ row, column }">
                  <vxe-input v-model="row.billCodes" type="text"> </vxe-input>
                </template>
              </vxe-column>
              ...
              >
                ...
                <template #default="{ row }">
                  <span>{{
                    row.stotageTaxRate ? row.stotageTaxRate + "%" : ""
                  }}</span>
                </template>
              </vxe-column>
              <vxe-column
                ...
                <template #edit="{ row, column }">
                  <div style="display: flex">
                    <n-image
                      ...
                      height="30"
                      v-if="row.fileIds"
                      :src="getUrl(row.fileIds)"
                    />
                    <fileUploadButton
                      v-model:fileIds="row.fileIds"
                      :showFileList="false"
                    >
                      <n-button text>
              ...
              >
                ...
                <template #default="{ row }">
                  <n-button
                    ...
                    @click="
                      () => {
                        fileUploadButtonRef.fileModalShow();
                      }
                    "
                  >
                    查看
  ...
  <stockPending
    ref="stockPendingRef"
    :acceptanceId="dataModel.acceptanceId"
    :applyId="dataModel.applyId"
    @callback="callbackSetItem"
  ></stockPending>
  <assetsPick
    ref="assetsPickRef"
