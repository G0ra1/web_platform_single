    ...
    <n-grid>
      <n-grid-item :span="showSide ? 18 : 24" style="height: 650px">
        <n-spin :show="spinShow">
          ...
          <vxe-table resizable show-overflow highlight-current-row ref="xTable" :row-config="{ keyField: 'rowId' }"
            :height="'600px'" :data="emp.empList">
            ...
            <vxe-column field="assetsCode" title="资产编号" minWidth="135">
            </vxe-column>
            ...
            <vxe-table-column field title="操作" width="120">
              <template v-slot="{ row }">
                <vxe-button type="text" @click="currentChangeEvent(row)"
                  v-if="!multiple && !isAddedByEmpId(row.assetsDetailId)" status="primary">选择</vxe-button>
                <vxe-button type="text" @click="
                  () => {
                    if (emp.alreadyEmpList.length == maxNumber) {
                      emp.alreadyEmpList.pop()
                      emp.alreadyEmpList.unshift(row)
                    } else {
                ...
                <vxe-button type="text" @click="
                  () => {
                    emp.alreadyEmpList = emp.alreadyEmpList.filter(
                      (d) => d.assetsDetailId !== row.assetsDetailId
                    );
                  }
                " v-if="isAddedByEmpId(row.assetsDetailId)" status="warning">取消</vxe-button>
              </template>
            </vxe-table-column>
          </vxe-table>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const aTable = ref();
    const sonFn = (row, e = []) => {
      maxNumber.value = row.distributeNumber
      showModalRef.value = true;
      model.value = e ? e : [];
      leftSpinShow.value = true;
      if (props.multiple && e && e.length > 0) {
        emp.alreadyEmpList = e;
      } else {
        emp.alreadyEmpList = [];
      }
      console.log(emp.alreadyEmpList, 'emp.alreadyEmpList')
      if (props.showSide) {
    ...
    const isAddedByEmpId = (id) => {
      return emp.alreadyEmpList.map((d) => d.assetsDetailId).includes(id);
    };
