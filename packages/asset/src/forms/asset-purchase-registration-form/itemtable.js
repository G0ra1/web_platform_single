...
export const dataChangeEvent = ({ row, column }) => {
  if (row.registerNumber && row.registerAmount) {
    // 采购含税总价
    row.registerSumAmount = row.registerNumber * 1 * (row.registerAmount * 1);

    if (row.registerTaxRate) {
      // 采购总税额
      row.registerSumUntaxedAmount = (
        row.registerSumAmount /
        (1 + row.registerTaxRate / 100)
      ).toFixed(2);
      // 采购税额
      row.registerUntaxedAmount = (row.registerAmount / (1 + row.registerTaxRate / 100)).toFixed(
        2
      );
      // 采购未税总额
      row.registerSumTaxAmount = (row.registerSumAmount - row.registerSumUntaxedAmount).toFixed(
        2
      );
      // 采购未税单价
      row.registerTaxAmount = (
        row.registerAmount - row.registerUntaxedAmount
      ).toFixed(2);
    }
  }
  calculationAmount();
...
const calculationAmount = () => {
  ...
  dataModel.value.sumTotalAmount = sum(
    detailList.map((d) => d.registerSumAmount)
  ).toFixed(2);
  ...
  dataModel.value.sumTotalNumber = sum(
    detailList.map((d) => d.registerNumber)
  ).toFixed(2);
