...
window.DM_GET_VALUE = function () {
  if (dataModel.value.contractTime) {
    dataModel.value.contractTime = format(
      new Date(dataModel.value.contractTime),
      "yyyy-MM-dd hh:mm:ss"
    );
  }
  if (dataModel.value.contractCode) {
    assetsList.value = reduceData(assetsList.value).map((d) => {
      d.contractCode = dataModel.value.contractCode;
      return d;
    });
  }
  if (dataModel.value.supplierName) {
    assetsList.value = reduceData(assetsList.value).map((d) => {
      d.supplierName = dataModel.value.supplierName;
      d.supplierId = dataModel.value.supplierId;
      return d;
    });
  }
