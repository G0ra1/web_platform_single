...
window.DM_SET_VALUE = async function (v) {
  ...
  if (v.materialWithdrawalDetailList && v.materialWithdrawalDetailList.length > 0) {
    assetsList.value = enlargeData(v.materialWithdrawalDetailList);
  }
  dataModel.value = v;
};
// 设置表单字段权限
...
window.DM_GET_VALUE = function () {
  ...
  dataModel.value.materialWithdrawalDetailList = reduceData(assetsList.value);
