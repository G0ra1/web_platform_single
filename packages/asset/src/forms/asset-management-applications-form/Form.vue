    ...
    <n-form :model="dataModel" :rules="rules" ref="formRef" label-placement="left" label-align="left" :label-width="90"
      ...
      <n-grid :cols="12" :x-gap="24">
        ...
        <n-form-item-gi :span="6" label="总金额" path="sumTotalAmount">
          <n-input placeholder="总金额" v-model:value="dataModel.sumTotalAmount" disabled />
        </n-form-item-gi>
        <n-form-item-gi :span="6" label="总数量" path="sumTotalNumber">
          <n-input placeholder="总数量" v-model:value="dataModel.sumTotalNumber" disabled />
        </n-form-item-gi>

        <n-form-item-gi :span="6" label="购置类型" path="warehouseName">
          <NwDic dictCode="asset_type" v-model:value="dataModel.itemType" v-model:label="dataModel.itemTypeName" isSort
            type="checkbox"></NwDic>
        </n-form-item-gi>
        ...
        <n-form-item-gi :span="12" label="" path="" v-if="dataModel.itemTypeName && dataModel.itemTypeName.length > 0">
...
" size="large" animated style="margin: 0 -4px"
            pane-style="padding-left: 4px; padding-right: 4px; box-sizing: border-box;" :on-update:value="tabsChange">
