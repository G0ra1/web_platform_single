...
class FormModal extends AbstractForm {
    ...
    totalNum = computed(
        () => {
            let sum = 0;
            this.dataModel.value.assetDealDetailList.map((d: any) => {
                sum += d.dealNum;
            })
            this.dataModel.value.sumTotalNumber = sum;
            return sum;
        }
    )
    //回显列
    columns = [
        ...
        {
            field: 'itemType', minWidth: 70, title: '资产类型', editRender: { enabled: false }, slots: {
                default: ({ row }: any) => [h(
                    NTag,
        ...
        { field: 'unitName', minWidth: 70, title: '单位', editRender: { enabled: false }, slots: { edit: 'unitName' } },
        ...
        //{ field: 'stockNumber', minWidth: 100, title: '库存数量', editRender: { enabled: false }, slots: { edit: 'stockNumber' } },
        { field: 'usableNumber', minWidth: 70, title: '可用数量', editRender: { enabled: false }, slots: { edit: 'usableNumber' } },
        { field: 'dealNum', minWidth: 100, title: '处置数量', editRender: { enabled: true }, slots: { edit: 'dealNum' } },
    ]
    //选择框列
    doubleListCols = [
        //{ colKey: 'id', width: 200, align: 'center', title: () => h('B', { style: 'color: #000' }, 'id'), ellipsis: true },
        ...
        { colKey: 'unitName', width: 70, align: 'center', title: () => h('B', { style: 'color: #000' }, '单位'), ellipsis: true },
        ...
        { colKey: 'usableNumber', width: 70, align: 'center', title: () => h('B', { style: 'color: #000' }, '可用数量'), ellipsis: true },
