/**
 * @Description 资产借用出库
 * @author 云数网讯 lhy@netwisd.com
 * @date 2022-12-09 12:18:53
 */

import { ref, h } from 'vue'
import { RequestPaging, VxeHelper, NwFunctionPredefine, Db, Page } from '@platform/main'
import { NTag } from 'naive-ui'
import { format } from "date-fns";
import { auditStatusArray,auditStatusTypeArray } from "../../lib/constant/Constant";

/**
 * 初始化弹出框组件
 */
let dialogLocal: any;
function initDialog(dialog: any) {
    dialogLocal = dialog;
}
/**
 * 列表的请求和组装，也可以直接在页面用使用vxe-grid形式
 */
const { bind, gridRef, filterData, query, reset } = new VxeHelper.VxeGridPaging(
    {
        columns: [
            { field: 'code', title: '编号', showHeaderOverflow: true, align: 'center'},
            { field: 'sumTotalAmount', title: '合计金额', showHeaderOverflow: true, align: 'center' },
            { field: 'sumTotalNumber', title: '合计申请数量', showHeaderOverflow: true, align: 'center' },
            { field: 'applyTime', title: '申请时间', showHeaderOverflow: true, align: 'center' },
            { field: 'applyUserName', title: '申请人', showHeaderOverflow: true, align: 'center' },
            { field: 'applyUserOrgName', title: '申请机构', showHeaderOverflow: true, align: 'center' },
            { field: 'applyUserDeptName', title: '申请部门', showHeaderOverflow: true, align: 'center' },
            {
                field: 'auditStatus', title: '审批状态', showHeaderOverflow: true, align: 'center',
                slots: {
                  default: ({ row }) => [h(
                    NTag,
                    { size: "small", type: auditStatusTypeArray[row.auditStatus] } as {},
                    { default: () => auditStatusArray[row.auditStatus] }
                  )] as JSX.Element[]
                }
            },
            {
                title: '操作',
                showHeaderOverflow: true,
                align: 'center',
                slots: {
                    default: ({ row }) => {
                        // let edit = h(
                        //     NwFunctionPredefine,
                        //     { code: "materialBorrowStorageFormEdit", params: [row] }
                        // );
                        let del = h(
                            NwFunctionPredefine,
                            { code: "materialBorrowStorageFormDel", params: [row] }
                        );
                        //return row.camundaProcinsId !== "" ? [edit] : [edit, del]
                        // return [ edit,del]
                        return row.auditStatus !== 0 ? [] : [del]
                    }
                },
            }
        ]
    }, new RequestPaging(
        '/asset/materialBorrowStorage/page',
        'post',
    )
)

const refresh = () => {
    reset({deployId: Page.getMenuData().id,type: 2})
}

/**
 * 传递用户参数例子
 */
 let paramsArray = ref([{}]);
 Db.get('userInfo').then((d: any) => {
     paramsArray.value = [{
         code: d.code, 
         applyUserId: d.id,
         applyUserName: d.userName, 
         applyUserDeptId: d.parentDeptId,
         applyUserDeptName: d.parentDeptName,
         applyUserOrgId: d.parentOrgId,
         applyUserOrgName: d.parentOrgName, 
         applyTime: format(new Date(), "yyyy-MM-dd HH:mm:ss"),
         createUserName: d.userName,
         createUserParentDeptName: d.parentDeptName, 
         createUserParentOrgName: d.parentOrgName, 
         createTime: format(new Date(), "yyyy-MM-dd HH:mm:ss"),
         deployId: Page.getMenuData().id,
         type: 2,
         typeName: "借用入库"
     }];
 })

export {
    bind, gridRef, filterData, query, refresh, reset, paramsArray, initDialog
}