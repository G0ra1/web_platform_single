...
const { bind, gridRef, filterData, query, reset } = new VxeHelper.VxeGridPaging(
    {
        columns: [
            ...
            { field: 'deployCode', title: '借用编号', showHeaderOverflow: true , align: 'center' },
            ...
            },
            {
                ...
                slots: {
                    default: ({ row }) => {
                        // let edit = h(
                        //     NwFunctionPredefine,
                        //     { code: "materialBorrowDeliveryFormEdit", params: [row] }
                        // );
                        let del = h(
                            NwFunctionPredefine,
                            { code: "materialBorrowDeliveryFormDel", params: [row] }
                        );
        ...
        '/asset/materialBorrowDelivery/page',
        'post',
    )
)
 ...
 */
 ...
 Db.get('userInfo').then((d: any) => {
     paramsArray.value = [{
         ...
         deployCode: Page.getMenuData().code,
