...
import { itemType } from "./api/index.js";
import {
  NDrawer,
  NDrawerContent,
  NLayout,
...
export default defineComponent({
  ...
  setup() {
    ...
    const columns = reactive([
      ...
      { field: "applyCode", title: "申请单号", showHeaderOverflow: true, showOverflow: true, },
      { field: "itemName", title: "物资名称", showHeaderOverflow: true, showOverflow: true, },
      { field: "itemCode", title: "物资编码", showHeaderOverflow: true, showOverflow: true, },
      {
        field: "", title: "购置类型", showHeaderOverflow: true, showOverflow: true, slots: {
          default: ({ row }) => {
            return <span>{itemTypeList.value.find(d => d.dictItemCode == row.itemType).dictItemName}</span>
          }
        }
      },
      ...
      { field: "applyAmount", title: "申请单价", showHeaderOverflow: true, showOverflow: true, },
      { field: "applySumAmount", title: "申请总价", showHeaderOverflow: true, showOverflow: true, },
      { field: "applyNumber", title: "申请数量", showHeaderOverflow: true, showOverflow: true, },
      { field: "registerNumber", title: "采购数量", showHeaderOverflow: true, showOverflow: true, },
    ...
    const renderSuffix = ({ option }) => {
      ...
      if (option.id != treeSelectId.value) {
        ...
        return <span>
          <NwIcon name="icon-bianji3" style="
          :10px" onClick={(e) => { e.stopPropagation(); editorg12(option) }} size={16}></NwIcon>
          <NPopconfirm
