    ...
    <n-drawer-content title="物资详情">
      <div class="manhour-details">
        ...
        <div class="manhour">
          ...
          <div>
            <span>申请说明</span><span>{{ dataModel.explanation }}</span>
          </div>
          <div>
          ...
          <vxe-column field="signStatus" title="签收状态" width="135" :edit-render="{}">
            <template #edit="{ row }">
              <vxe-select v-model="row.signStatus" transfer>
                <vxe-option :key="0" :value="0" label="未签收">
                </vxe-option>
                <vxe-option :key="1" :value="1" label="已签收">
                </vxe-option>
              </vxe-select>
            </template>
...
import { getpurchaseRegister, fileinfo } from "./api/index.js";
import {
  defineComponent,
  ref,
  reactive,
