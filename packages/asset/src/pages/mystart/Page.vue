<template>
  ...
  <n-modal v-model:show="showTaskStepModal" style="width:1000px;height:600px" :mask-closable="false" preset="dialog"
    title="任务日志" positive-text="确认" negative-text="取消">
    <n-data-table style="height:480px" striped :loading="taskStepLoading" :columns="taskStepcolumns"
      :data="taskStepdata" :pagination="pagination" :bordered="false" />
  </n-modal>
</template>
...
import { getTaskStep, } from './api/index.js'
...
export default defineComponent({
  ...
  setup() {
    ...
    const columns = reactive([
      { type: 'checkbox', width: 60, minWidth: 60 },
      {
        field: "taskState", title: "状态", showHeaderOverflow: true, showOverflow: true, slots: {
          default: ({ row }) => {
            return <NTag onClick={() => taskStep(row)} type={row.taskState == 0 ? 'warning' : row.taskState == 1 || row.taskState == 3 ? '' : 'success'} >
    ...
    const showTaskStepModal = ref(false)
    const taskStepcolumns = reactive([
      { title: '单据编号', key: "applyCode", width: 160 },
      { title: '单据类型', key: "taskSource", width: 80 },
      { title: '标题', key: "taskName", width: 180 },
      { title: '办理人', key: "personName", width: 80 },
      { title: '创建时间', key: "createTime", width: 180 },
      { title: '任务说明', key: "applyReason", width: 180 },
      {
        title: '是否待办', key: "isTodo", width: 80, render: (rowData) => {
          return <span>{rowData.isTodo == 0 ? '否' : '是'}</span>
        }
      }

    ])
    const taskStepdata = ref([])
    const taskStepLoading = ref(false)
    const taskStep = (row) => {
      showTaskStepModal.value = true
      taskStepLoading.value = true
      getTaskStep(row.id).then(res => {
        taskStepdata.value = res
      }).finally(e => {
        taskStepLoading.value = false
      })
    }
