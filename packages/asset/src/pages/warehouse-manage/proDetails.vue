    ...
    <n-drawer-content title="仓库详情">
      <div class="manhour-details">
        ...
        <div class="taskShow">
          <div>
            <h2>仓库名称</h2>
            <span>{{ dataModel.warehouseName }}</span>
          </div>
          <div>
            <h2>仓库类型</h2>
            <span>{{ formatHouseType(dataModel.houseType) }}</span>
          </div>
        </div>
        <div class="manhour">
          <h3>仓库详细信息</h3>
          <div>
            <span>责任主体</span><span>{{ dataModel.orgName }}</span>
          </div>
          <div>
            <span>责任部门</span><span>{{ dataModel.deptName }}</span>
          </div>
          <div>
            <span>责任人</span><span>{{ dataModel.respondUserName }}</span>
          </div>
          <div>
            <span>地点</span
            ><span style="width: auto">{{ dataModel.address }}</span>
            <span>备注</span
            ><span style="width: auto">{{ dataModel.remark }}</span>
          </div>
        </div>
      </div>
      <goodsShelf
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const showShelf = (row = dataModel.value) => {
