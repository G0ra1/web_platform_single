...
export default defineComponent({
  ...
  setup() {
    ...
    const columns = reactive([
      ...
      {
        field: "code", title: "签收编号", showHeaderOverflow: true, showOverflow: true, slots: {
          default: ({ row }) => {
            return <span onClick={() => { showDetails(row) }} style="color:#1d7ced;cursor: pointer;">{row.code}</span>
          }
        }
      },
      { field: "applyUserOrgName", title: "申请单位", showHeaderOverflow: true, showOverflow: true, },
      { field: "applyUserDeptName", title: "申请部门", showHeaderOverflow: true, showOverflow: true, },
