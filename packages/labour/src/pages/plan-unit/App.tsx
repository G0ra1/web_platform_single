...
const { bind, gridRef, filterData, query, reset,gridEvents } = new VxeHelper.VxeGridPaging(
    {
        columns: [
                        ...
                        return h(
                            "a",
                            {
                                ...
                                onClick: () => {
                                    ...
                                    Page.toLevel2Menu('id', row.id, 'planName', row.planName, JSON.stringify(row))
                                }
                            },
            ...
            {
                field: 'auditStatus',title: '审批状态', showHeaderOverflow: true,showOverflow:true,width: '160px',align: 'center',
                slots: {
                    default: ({ row }) => [h(
                        NTag,
                        {
            ...
            { field: 'overhaulName', title: '大修轮次', showHeaderOverflow: true, width: '160px',showOverflow:true, align: 'center' },
            { field: 'estimateAmount', title: '预估金额', showHeaderOverflow: true, width: '160px',showOverflow:true, align: 'center', formatter: 'myAmount' },
            // { field: 'budgetAmount', title: '预算金额', showHeaderOverflow: true, width: '160px',showOverflow:true, align: 'center', formatter: 'myAmount' },
            ...
            {
                field: 'isInvalid', title: "是否作废", showHeaderOverflow: true,showOverflow:true, width: '160px', align: 'center',
                slots: {
                    default: ({ row }) => [h(
                        NTag,
                        { size: "small", type: typeArray[row.isInvalid] } as {},
                        { default: () => statusArray[row.isInvalid] }
                    )] as JSX.Element[]
                }
            },
            { field: 'pushOffTime', title: '推送截止时间', showHeaderOverflow: true, showOverflow:true,width: '160px', align: 'center' },
            {
                ...
                slots: {
                    default: ({ row }) => {
                        let edit = h(
                            NwFunctionPredefine,
                            { code: "labourPlanForm_edit", params: [row], text: true }
                        );
                        let del = h(
                            NwFunctionPredefine,
                            { code: "labourPlanForm_del", params: [row], text: true }
                        );
                        let _cancle = h(
                            NwFunctionPredefine,
                            { code: "labourPlanForm_cancle", params: [row], text: true }
                        );
                        let genOrder = h(
                            NwFunctionPredefine,
                            { code: "labourPlanForm_genOrder", params: [row], text: true }
                        );
                        if (!(row.auditStatus == "2" && row.isGenOrder != "1")) {

                        }else{
                            let _change = h(
                                NwFunctionPredefine,
                                {
                                    code: "labourPlanForm_change",
                                    params: [row],
                                    pid: row.id,
                                    text: true
                                }
                            );
                            return [edit, del, _change, _cancle, genOrder]
                        }
                        return [edit, del, _cancle, genOrder]
                    }
                },
            }
        ]
...
const refresh = () => {
     filterData.value = {planType:'',planOrgName:'',auditStatus:'',isGenOrder:''};
    reset({planType:'',planOrgName:'',auditStatus:'',isGenOrder:''})
}
//所属机构名称 选项
