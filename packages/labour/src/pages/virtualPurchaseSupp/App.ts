...
const { bind, gridRef, filterData, query, reset, gridEvents } = new VxeHelper.VxeGridPaging(
    {
        columns: [
            ...
            {
                field: 'datasource', title: '数据来源', showHeaderOverflow: true, width: '160px', align: 'center',
                slots: {
                    default: ({ row }) => [h(
                        NTag,
                        { size: "small", type: "success" } as {},
                        { default: () => sourceMap.get(row.datasource) }
                    )] as JSX.Element[]
                }
            },
        ...
        '/labour/labourVirtualPurchase/suppPage',
        'post'
    )
)
...
const createReferral = async (c: any, d: any) => {
    ...
    refePurchase.virtualPurchaseId = c.id
...
export {
    bind, gridRef, filterData, query, refresh, reset, paramsArray, initDialog, createReferral,gridEvents
}