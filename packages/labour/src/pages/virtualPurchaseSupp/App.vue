            ...
            <n-layout-content>
                <vxe-grid v-bind="bind" ref="gridRef">
                    ...
                    <NwPickAny
                        ...
                        button-label="关联采购合同"
                        :buttonOptions="{
                        disabled: row.isLinkPurchase == 1,
                        type: 'primary',
                        }"
                        modal-title="关联采购合同"
                        isSingle
                        :value="row.contractcode"
                        @update:value="(d)=>{createReferral(row,d)}"
                        :gridColumns="[
                    ...
                    <NwFunctionPredefine code="statement" :params="[row]" pid = "row.id"/>
                    </template>
...
import { NLayout, NLayoutHeader, NLayoutContent, NSpace, NInput, NSelect, NInputGroup, useDialog, NButton,NForm,NFormItem } from 'naive-ui'
import { bind, gridRef, filterData, query, refresh, reset, paramsArray, initDialog,createReferral,gridEvents } from "./App"
