        ...
        <n-layout class="nw-layout-full">
            ...
            <n-layout-header class="nw-layout-header">
                <n-space :size="5" style="padding: 0 5px">
                    <!--放功能按钮的地方-->
                    <NwFunctionPredefine code="purchaseExtendForm_add" :params="paramsArray" />
                </n-space>
                <n-space :size="5" style="padding: 0 5px">
                    <n-input-group>
                        <n-input :value="filterData.contractId || ''" @update:value="(d) => filterData.contractId = d"
                            placeholder="采购合同ID" :style="{ width: '150px' }" style="margin-right: 10px" />
                        <n-input :value="filterData.taxrateName || ''" @update:value="(d) => filterData.taxrateName = d"
                            placeholder="税率" :style="{ width: '150px' }" style="margin-right: 10px" />
                        <n-input :value="filterData.taxrate || ''" @update:value="(d) => filterData.taxrate = d"
                            placeholder="税率" :style="{ width: '150px' }" style="margin-right: 10px" />
                        <n-input :value="filterData.invoiceTypeName || ''" @update:value="(d) => filterData.invoiceTypeName = d"
                            placeholder="发票类型" :style="{ width: '150px' }" style="margin-right: 10px" />
                        <n-input :value="filterData.invoiceType || ''" @update:value="(d) => filterData.invoiceType = d"
                            placeholder="发票类型" :style="{ width: '150px' }" style="margin-right: 10px" />
                        <n-input :value="filterData.applyTime || ''" @update:value="(d) => filterData.applyTime = d"
