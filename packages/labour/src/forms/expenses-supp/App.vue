      ...
      <n-card title="基本信息" :hoverable="true" :embedded="true">
        ...
        <n-grid :cols="24" :x-gap="24">
          <n-form-item-gi :span="8" label="大修计划:">
            <n-gradient-text type="info">{{ dataModel.overhaulName }}</n-gradient-text>
          </n-form-item-gi>
          ...
          <n-form-item-gi :span="8" label="填报时间:" path="fillDateTime"
            ...
            <NDatePicker placeholder="填报时间" :disabled="dataPermits.fillDateTime === FormPermissionEnum.READONLY"
              type="date" format='yyyy-MM-dd' :formattedValue="dataModel.fillDateTime"
              @update:formatted-value="(val) => dataModel.fillDateTime = val" />
          </n-form-item-gi>
        </n-grid>
        <n-grid :cols="24" :x-gap="24">
          <n-form-item-gi :span="16" label="承包合同:" path="contractWorkName"
            ...
            <n-select placeholder="承包合同" filterable :render-option="contractWorkRender"
              :disabled="dataPermits.contractWorkName === FormPermissionEnum.READONLY"
              v-model:value="dataModel.contractWorkName" label-field="workContractName" value-field="workContractName"
              :options="contractWorkArray" @update-value="upContractWorkMeth" />
          </n-form-item-gi>
        </n-grid>
      </n-card>
            ...
            <n-tabs type="line" animated>
              <n-tab-pane name="travelList" tab="差旅费" display-directive="show">
                <NwFieldGrid v-bind="SubColumnOptions" :columns="TravelColumn"
                  ...
                  <template #buttons>
                    <NwPickAny rowKey="id" button-label="选择" modal-title="劳务人员" :width="1000" :height="600"
                      :gridColumns="SuppUserChooseColumn" :value="dataModel.travelList"
                      @update:value="rows => addPerson(rows, 'travel')" :requestUrl="AddPersonReqUrl" :requestParam="{
              ...
              <n-tab-pane name="nucleicacidList" tab="核酸检测费" display-directive="show">
                <NwFieldGrid v-bind="SubColumnOptions" :columns="NucleicacidColumn"
                  ...
                  <template #buttons>
                    <NwPickAny rowKey="id" button-label="选择" modal-title="劳务人员" :width="1000" :height="600"
                      :gridColumns="SuppUserChooseColumn" :value="dataModel.nucleicacidList"
                      @update:value="rows => addPerson(rows, 'nucleicacid')" :requestUrl="AddPersonReqUrl"
                      :requestParam="{
                        virtualPurchaseId: dataModel.virtualPurchaseId,
                      }">
              ...
              <n-tab-pane name="examinationList" tab="体检费" display-directive="show">
                <NwFieldGrid v-bind="SubColumnOptions" :columns="ExaminationColumn"
                  :tooltip-config="{ showAll: true, enterable: true }"
                  :ref="(d) => gridRefs['examinationList'] = d" v-model:value="dataModel.examinationList"
                  :rules="gridRules['examinationList']" :edit-enabled="true"
                  :data-permits="gridDataPermits['examinationList']">
                  <template #buttons>
                    <NwPickAny rowKey="id" button-label="选择" modal-title="劳务人员" :width="1000" :height="600"
                      :gridColumns="SuppUserChooseColumn" :value="dataModel.examinationList"
                      @update:value="rows => addPerson(rows, 'examination')" :requestUrl="AddPersonReqUrl"
                      :requestParam="{
                        virtualPurchaseId: dataModel.virtualPurchaseId,
                      }">
                      <template #search = "{ searchData }">
