...
class FormModal extends AbstractForm {
  ...
  printScript=(params: Array<any>) => {
    let planCpt="http://10.72.5.45:9095/webroot/decision/view/report?viewlet=expensesFiliale.cpt";
  ...
  addFestivalUser = (rows: []) => {
    ...
    this.dataModel.value.labourExpensesFilialeFestivalList = rows.map((row: any) => {
      ...
      return { suppUserId, suppUserName,idCard,suppOrgId,suppOrgCode,suppOrgName,banId,banName,festivalCost: isNaN(festivalCost) ?  1000 : festivalCost,total,remark,attachmentids,attachment }
    }) as [];
  }
