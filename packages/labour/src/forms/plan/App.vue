...
import { FormModal,contractNameOptions,contractCodeOptions,contractOwnercompanynameOptions,contractOrgannameOptions,overhaulOrgnameOptions,typeOptions } from "./App";
import { getDict } from "./api";
import { FormPermissionEnum } from "../../lib/constant/FormPermissionEnum";
import { RestModeOptions, RestModeArry, UnitOptions, UnitArry,WorkContractColumns } from './constant'
...
<template> 
    <n-form ref="formRef" class="LForm" :model="dataModel" :rules="rules" size="medium" label-placement="left" label-width="150"
        ...
        <n-grid :cols="24" :x-gap="24" class="LFormNg" >
                        ...
                        <template #search = "{ searchData }">
                            <n-form label-placement="left" :show-feedback="false" inline>
                                <n-form-item label="年份:">
                                    <n-input v-model:value="searchData.year" placeholder="年份" size="medium" />
                                </n-form-item>
            ...
            <n-form-item-gi  :span="24" path="labourPlanContractWorkList" class="LFormTable"  v-if="dataPermits.labourPlanContractWorkList !== FormPermissionEnum.HIDE">
                <NwFieldGrid
                    v-model:value="dataModel.labourPlanContractWorkList"
                    :is-readonly="(dataPermits.labourPlanContractWorkList === FormPermissionEnum.READONLY)"
                    style="width: 100%"
                    :edit-enabled="false"
                    :columns="[
                ...
                >
                    ...
                    <NwPickAny
                        ...
                        "
                        @update:value="(d) => {
                            dataModel.labourPlanContractWorkList = d.map((m: any) => ({ 
                                workContractCode: m.contractcode,
                                workContractName: m.contractname,
                                workContractId:m.id 
                                })
                            )}"
                        :gridColumns="WorkContractColumns"
                        ...
                        <n-form label-placement="left" :show-feedback="false" inline> 
                            ...
                            <n-form-item label="合同类型:">
                                <n-select
                                    ...
                                    :options="typeOptions"
                                    @update:value="(d)=>{searchData.type = d }"
                                    :style="{
                                        width: '80px' 
                                    }"
                                    @select="(d)=>{}"
            ...
            <n-form-item-gi  :span="24" path="labourPlanSuppOrgList" class="LFormTable">
                <NwFieldGrid
                    v-model:value="dataModel.labourPlanSuppOrgList"
                    style="width: 100%"
                    :edit-enabled="false"
                    :columns="[
                ...
                >
                    ...
                    <NwPickAny
                        ...
                        modal-title="供应商"
                        :value="
                        (dataModel.labourPlanSuppOrgList || []).map((d) => ({
                            orgCode: d.suppOrgCode,
                            orgName: d.suppOrgName,
                            orgShortName:d.suppOrgShortName,
                            id: d.suppOrgId,
                        }))
                        "
                        @update:value="(d) => {dataModel.labourPlanSuppOrgList = d.map((m: any) => ({ suppOrgId: m.id, suppOrgCode: m.orgCode, suppOrgName:m.orgName, suppOrgShortName:m.orgShortName }))}"
                        :gridColumns="[
            ...
            >
                <NwFieldGrid
                    :height="500"
                    :ref="(d) => gridRefs['labourPlanPersonList'] = d"
