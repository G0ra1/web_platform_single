<template>
  ...
  <n-modal style="width: 650px; padding: 0" v-model:show="visible" :mask-closable="true" :closable="true"
    ...
    <n-upload multiple directory-dnd :action="fileUploadAction" :show-download-button="true" response-type="json"
      ...
      @finish="({ file, event }: any) => fileUploadFinish({ file, event })"
      @error="({ file, event }: any) => fileUploadError({ file, event })"
      @remove="({ file, fileList }: any) => fileUploadRemove({ file, fileList })"
...
//文件上传地址
let fileUploadAction = ref(`${window.apiBaseURL}/main/fileinfo?fileSource=expenseSupp`);
